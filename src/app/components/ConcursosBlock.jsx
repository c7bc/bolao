...
export default function LotteryPools() {
  ...
  const handleSearch = () => {
    const filters = {
      ...
      player: (ticket) =>
        ticket.player.toLowerCase().includes(searchTerm.toLowerCase()),
      numbers: (ticket) =>
  ...
  const renderSearchButtons = () => {
    return searchButtons.map(({ label, value }) => (
      <Button
        ...
        _hover={{
          borderColor: searchType === value ? "green.400" : "gray.300",
        }}
      >
        {label}
      </Button>
      ...
      >
        <Box>
          <Flex align="center" gap={2} mb={2}>
            ...
            <Badge
              colorScheme={pool.status === "open" ? "green" : "yellow"}
              fontSize="sm"
            >
          ...
          <Stack spacing={1}>
            <Text color="green.700">
              Valor da entrada: {pool.entryValue}
            </Text>
            <Text color="green.700">Prêmio: {pool.prizeValue}</Text>
            <Text color="green.700">
              Pontos necessários: {pool.requiredPoints}
            </Text>
          </Stack>
        </Box>
        <Flex
        ...
        >
          ...
          <Link href={`/bolao/${pool.slug}`} passHref>
            <Button
              colorScheme="white"
              color="green.600"
              size="lg"
        ...
        <Td>
          <Flex gap={1} flexWrap="wrap">
            {ticket.numbers.map((num) => (
              <Badge key={num} colorScheme="green" variant="subtle">
                {num}
              </Badge>
            ))}
          </Flex>
  ...
  return (
    <Container maxW={containerMaxWidth} py={12}>
      {" "}
      {/* Aumentei o padding vertical para mais espaçamento */}
      ...
      <Box mb={8}>
        {renderPools().map((pool, index) => (
          <Box key={pool.id} mb={4}>
            {" "}
            {/* Aqui adicionei a margem inferior aos cards */}
            {pool}
          </Box>
        ))}
      </Box>
      <Box overflowX="auto" mb={12}>
        {" "}
        {/* Aumentando a margem inferior aqui para espaçamento extra */}
        <Heading as="h2" size="xl" color="green.800" textAlign="center" mb={8}>
          Filtros
        </Heading>
        <Flex
        ...
        {searchType && (
          <Flex gap={4} mb={6} align="center">
            ...
            />
            <Button onClick={handleSearch} colorScheme="green" mb={4}>
              {" "}
              {/* Aumentei a margem inferior para mais espaçamento */}
              Buscar
            </Button>
            <Button
              onClick={() => {
      ...
      >
        <ModalOverlay />
        <ModalContent borderRadius="lg" boxShadow="lg" p={5} bg="gray.50">
          <ModalHeader
            fontSize="2xl"
            fontWeight="bold"
            textAlign="center"
            ...
            {selectedTicket && (
              <VStack align="flex-start" spacing={4}>
                <HStack>
                  ...
                  <Text fontSize="lg">{selectedTicket.id}</Text>
                </HStack>
                <HStack>
                  <Icon as={FaUser} color="green.400" boxSize={5} />
                  <Text fontSize="lg" fontWeight="semibold">
                    Apostador:
                  </Text>
                  <Text fontSize="lg">{selectedTicket.player}</Text>
                </HStack>
                <HStack>
                  <Icon as={FaCity} color="green.400" boxSize={5} />
                  <Text fontSize="lg" fontWeight="semibold">
                    Cidade:
                  </Text>
                  <Text fontSize="lg">{selectedTicket.city}</Text>
                </HStack>
                <HStack>
                  ...
                  <Text fontSize="lg">{selectedTicket.numbers.join(", ")}</Text>
                </HStack>
                <HStack>
                  <Icon as={FaRegClock} color="green.400" boxSize={5} />
                  <Text fontSize="lg" fontWeight="semibold">
                    Pontos:
                  </Text>
                  <Text fontSize="lg">{selectedTicket.currentPoints}</Text>
                </HStack>
                <HStack>
                  <Icon as={FaGift} color="green.400" boxSize={5} />
                  <Text fontSize="lg" fontWeight="semibold">
                  ...
                  <Text fontSize="lg">{selectedTicket.poolTitle}</Text>
                </HStack>
                <HStack>
                  <Icon as={FaRegCalendarAlt} color="green.400" boxSize={5} />
                  <Text fontSize="lg" fontWeight="semibold">
                    Início:
                  </Text>
                  <Text fontSize="lg">
                    {new Date(selectedTicket.poolStartTime).toLocaleString()}
                  </Text>
                </HStack>
                <HStack>
                  <Icon as={FaRegCalendarAlt} color="green.400" boxSize={5} />
                  ...
                  <Text fontSize="lg">
                    {selectedTicket.poolStatus === "open"
                      ? "Em andamento"
                      : "Em breve"}
                  </Text>
                </HStack>
                <HStack>
                  <Icon as={FaPhone} color="green.400" boxSize={5} />
                  <Text fontSize="lg" fontWeight="semibold">
                    Telefone:
                  </Text>
                  <Text fontSize="lg">{selectedTicket.phone}</Text>
                </HStack>
                <HStack>
                  <Icon as={FaRegCalendarAlt} color="green.400" boxSize={5} />
                  <Text fontSize="lg" fontWeight="semibold">
      ...
      <Flex justify="center" mt={8} gap={4}>
        {" "}
        {/* Aumentando a margem superior para mais espaçamento */}
        <IconButton
